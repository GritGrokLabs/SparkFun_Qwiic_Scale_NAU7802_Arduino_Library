...
public:
  ...
  bool begin(i2c_port_num_t port_num); // Check communication and initialize sensor
...
private:
  ...
  // Write a single byte to a register using the new I2C driver
  esp_err_t writeRegister(uint8_t reg, uint8_t value);

  // Read multiple bytes from registers using the new I2C driver
  esp_err_t readRegisters(uint8_t reg, uint8_t *data, size_t len);

  i2c_master_dev_t *i2cDevice;
  i2c_master_bus_t *i2cMaster;
};
